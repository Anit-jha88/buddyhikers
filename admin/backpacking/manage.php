...
<div class="col-lg-12">
	<div class="card card-outline card-primary">
		<div class="card-header">
			<h5 class="card-title"><?php echo isset($id) ? "Manage": "Create" ?> New Backpacking</h5>
		</div>
		<div class="card-body">
			<form id="allproject" enctype="multipart/form-data">
			    	<input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
			    	<input type="hidden" name="postcategory" value="Backpacking">
			    ...
			    <h4>Details</h4><hr>
					...
					<div class="col-sm-4">
						<div class="form-group">
							<label for="" class="control-label">Backpacking Name</label>
				...
				<h4>Upload Images</h4><hr>
				 ...
				 <h4>Itinerary</h4><hr>
				  <?php
					 ...
					 $n=1;
					 $qry = $conn->query("SELECT * FROM `Itinerary`  where commonid = '{$_GET['id']}' AND `Itineraryfor`='Backpacking' " );
					 $num=$qry->num_rows;
					 if($num>0){
			   ...
			   <div class="field_wrapper1">
				...
				<h4>Inclusions & Exclusions</h4></hr>
					 ...
					 $n=1;
					 $qry = $conn->query("SELECT * FROM `includeandexclude`  where commonid = '{$_GET['id']}' AND `includeandexcludefor`='Backpacking' " );
					 $num=$qry->num_rows;
					 if($num>0){
					 while($row= $qry->fetch_assoc()):
				...
				<?php } ?>
					 ...
					 $n=1;
					 $qry = $conn->query("SELECT * FROM `batches`  where commonid = '{$_GET['id']}' AND `batchesfor`='Backpacking' " );
					 $num=$qry->num_rows;
					 if($num>0){
					 while($row= $qry->fetch_assoc()):
				...
				success:function(resp){
					if(resp != undefined){
						if(resp.status == 'success'){
							location.href=_base_url_+"admin/?page=backpacking";
						}else{
							alert_toast("An error occured",'error')
							console.log(resp);
						}
